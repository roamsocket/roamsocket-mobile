...
const scratch = process.env.APC_SCRATCH || path.join(root, '.vite'); // fallback; harness sets APC_SCRATCH
...
function assertSnapshot(s: DomSnapshot, label: string): void {
  ...
  console.log(
    `${label}: OK nav=${routes.join(',')} greeting=${JSON.stringify(s.greeting)} empty=${s.emptyHome}`
  );
}
...
function runOnce(logPath: string, attempt: number): Promise<DomSnapshot> {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      child.kill('SIGKILL');
      reject(
        new Error(`timeout waiting for DOM_SNAPSHOT (attempt ${attempt})\n${buf.slice(-2000)}`)
      );
    }, 45_000);
    ...
    child.on('exit', () => {
      ...
      if (!settled) {
        ...
        reject(
          new Error(
            `electron exited without DOM_SNAPSHOT (attempt ${attempt})\n${buf.slice(-2000)}`
          )
        );
      }
    });
