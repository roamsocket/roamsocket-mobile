...
assert.ok(
  main.includes('populateModelPickerList') || main.includes('listCloudModels'),
  'honest model picker'
);
assert.ok(
  main.includes('hasUsableChatModel') || main.includes('isUsableChatSelection'),
  'usable model gate'
);
assert.ok(
  !/state\.model = p\.defaultModel/.test(main),
  'picker must not force static defaultModel'
);
...
assert.ok(
  !/if\s*\(\s*usable\s*\)\s*openModelPicker\(\)\s*;\s*else\s*\{[^}]*settingsTab\s*=\s*"providers"/.test(
    main
  ),
...
assert.ok(
  main.includes('installRuntime') || main.includes('runMetalRuntimeInstall'),
...
assert.ok(
  main.includes('contextItems') || main.includes('addContextItem'),
  'context items store use'
);
...
assert.ok(
  composerMenu.includes('Skills') && composerMenu.includes('Connectors'),
  'skills/connectors'
);
assert.ok(
  composerMenu.includes('Plugins') && composerMenu.includes('Research'),
  'plugins/research'
);
...
console.log(
  'composer +: files / project / github / skills / connectors / plugins / research / web'
);

// Skills slash chips + detail UI
const skillUi = readFileSync(path.join(root, 'src/renderer/skill-ui.ts'), 'utf8');
assert.ok(
  skillUi.includes('skill-slash-chip') || skillUi.includes('buildSkillSlashChip'),
  'slash chip'
);
...
assert.ok(
  css.includes('skill-detail-modal') || css.includes('skill-detail'),
  'skill detail styles'
);
...
assert.ok(
  main.includes('fillSettingsMarketplace') || main.includes('marketplace'),
...
const projectModalsSrc = readFileSync(path.join(root, 'src/renderer/project-modals.ts'), 'utf8');
assert.ok(
...
assert.ok(
  main.includes('fillSettingsSkills') && main.includes('skillsStore.list'),
  'settings skills list store'
);
assert.ok(
...
assert.ok(
  main.includes('baseUrl: ep?.baseUrl') || main.includes('baseUrl: codeEp.baseUrl'),
  'chat/code pass baseUrl'
);
...
console.log(
  'custom providers: settings CRUD + picker + baseUrl/apiStyle on chat and create_session'
);
...
assert.ok(
  main.includes('appPrompt') || main.includes('appForm'),
  'main uses electron-safe dialogs'
);
