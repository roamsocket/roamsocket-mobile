...
import { parseMetalArgs, resolveMetalHubQuery, metalHelpText } from '../src/cli/metal-cli.js';
...
import { describeProjectMemory, readProjectConfig } from '../src/project/config.js';
...
async function main() {
    ...
    try {
      ...
      const writeResults = messages.filter(
        (m) =>
          m.type === 'tool_result' &&
          messages.some(
            (c) => c.type === 'tool_call' && c.callId === m.callId && c.tool === 'write_file'
          )
  ...
  await test('isDirectMain: bin path must not auto-run (import-only)', async () => {
    ...
    const mainSrc = await readFile(path.join(process.cwd(), 'src/cli/main.ts'), 'utf8');
