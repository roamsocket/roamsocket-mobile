...
export function getMetalBrowserSnapshot(): MetalBrowserSnapshot {
  ...
  const families: MetalBrowserFamily[] = [...byFamily.keys()]
    ...
    .map((name) => {
      const models = (byFamily.get(name) ?? [])
        .slice()
...
export function formatMetalCatalogList(): string {
  ...
  const lines: string[] = [`Metal catalog (${rows.length} recommended) · store: ${store.root}`, ''];

  for (const e of rows) {
    const mark = e.downloaded ? '✓' : '·';
    lines.push(`${mark} ${e.displayName}  ${e.approxSize ?? ''}`.trimEnd());
    lines.push(`    ${e.hubID}`);
  ...
  lines.push('');
  lines.push(`${downloaded.length} downloaded${gb}.  /metal download <name>  ·  /metal use <name>`);
  return lines.join('\n');
}
 ...
 */
export function resolveMetalHubQuery(
  query: string
):
  ...
  const exactDl = downloaded.find(
    (m) => m.hubID === q || m.hubID.toLowerCase() === q.toLowerCase()
  );
  if (exactDl) {
  ...
  if (
    unique.length === 1 ||
    (unique[0]!.score >= 80 && unique[0]!.score > (unique[1]?.score ?? 0))
  ) {
    const h = unique[0]!;
...
function completeMetalTargets(
  ...
  if (opts.downloadedOnly) {
    ...
    entries = [...entries.filter((e) => !e.downloaded), ...entries.filter((e) => e.downloaded)];
  }

  const q = restQ.toLowerCase();
  const filtered = entries.filter(
    (e) => !q || e.displayName.toLowerCase().includes(q) || e.hubID.toLowerCase().includes(q)
  );
