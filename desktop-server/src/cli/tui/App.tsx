...
export function App({ session, server, mock, bus, permBridge, onQuit }: AppProps) {
  ...
  const applyEdit = useCallback((edit: { text: string; cursor: number }) => {
    setDraft(edit.text);
  ...
  const refreshModelCatalog = useCallback(async () => {
    ...
    setModelCatalog((prev) =>
      prev ? { ...prev, loading: true } : { linked: [], byProvider: {}, loading: true }
    );
    try {
  ...
  const permWaiters = useRef(new Map<string, (d: PermissionDecision) => void>());
  ...
  const handleSubmit = useCallback(
    async (raw: string) => {
      ...
      if ((stateRef.current.busy || sessionRef.current.isRunning) && !text.startsWith('/')) {
        return;
      }
      if (
      ...
      switch (cmd.kind) {
        ...
        case 'doctor': {
          const keyOk = mock || Boolean(resolveApiKey(sessionRef.current.currentModel.provider));
          const lines = [
        ...
        case 'init': {
          ...
          dispatch({
            type: 'system',
            text: result.created ? `Created ${result.path}` : `Already exists: ${result.path}`,
            level: 'info',
          });
          return;
        }
        ...
        case 'permission': {
          const mode = cmd.mode ?? cyclePermissionMode(sessionRef.current.currentPermissionMode);
        ...
        case 'model': {
          ...
          const modelId = cmd.model ?? secrets.model ?? sessionRef.current.currentModel.model;
  ...
  useInput((input, key) => {
    ...
    if ((key.leftArrow && wordMod && !key.super) || (key.meta && input === 'b')) {
      setCursor(moveWordLeft(draft, cursor));
      return;
    }
    if ((key.rightArrow && wordMod && !key.super) || (key.meta && input === 'f')) {
    ...
    if ((key.backspace && (key.meta || key.ctrl || key.super)) || (key.ctrl && input === 'w')) {
  ...
  const activeMetalHub =
    state.provider === 'localMetal' || state.provider === 'local-metal' ? state.model : null;
    ...
    <Box flexDirection="column" height={termRows} width={termCols} overflow="hidden">
      ...
      {completions.length > 0 && (
        <Box
          ...
          flexShrink={0}
        >
          <Text color={theme.muted}>Completions · Tab accept · ↑↓ cycle</Text>
          {completions.map((c, i) => (
            <CompletionRow
              key={`${c.token}-${i}`}
              item={c}
...
function formatItemForExport(item: StreamItem): string {
  switch (item.kind) {
    ...
    case 'tool': {
      const st = item.ok === true ? 'ok' : item.ok === false ? 'fail' : '…';
...
function StreamRow({ item }: { item: TuiState['items'][number] }) {
  switch (item.kind) {
    ...
    case 'thinking': {
      ...
      const preview = body
        ...
        : item.open
          ? 'Thinking…'
          : 'Thought process';
      return <Text color={theme.muted}>◐ {preview}</Text>;
    }
    case 'tool': {
      // Single-line Claude Code style: "● Write path · ok" — no borders, no dumps.
      const status = item.ok === true ? 'ok' : item.ok === false ? 'fail' : '…';
      const statusColor =
        item.ok === true ? theme.success : item.ok === false ? theme.error : theme.muted;
      ...
      return (
        <Text>
          ...
          {item.result ? <Text color={theme.error}> — {item.result}</Text> : null}
        </Text>
      );
    }
    case 'diff':
    ...
    case 'system': {
      const color =
        item.level === 'error' ? theme.error : item.level === 'warn' ? theme.warning : theme.muted;
