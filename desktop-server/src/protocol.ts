...
// E2B sandbox runs are now phone-only. The iOS / Android apps talk to
// e2b.dev directly with the user's own API key — see
// `ios/AnyProvCore/.../Sandboxes/DirectE2BClient.swift`. The desktop
// no longer brokers sandbox starts, lists, aborts, or key overrides,
// so the corresponding WS message types have been removed from the
// protocol.
 ...
 */
// E2B sandbox state was previously modelled here (`E2bRun` plus a
// family of `e2b_*` WS messages). E2B runs are now phone-only — the
// desktop is not in the loop. The phone's `E2bPhoneRun` /
// `E2bPhoneRunStep` models live in
// `ios/AnyProvCore/.../Sandboxes/DirectE2BClient.swift`. The
// corresponding Zod schemas have been removed from this file.
