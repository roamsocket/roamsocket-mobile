...
import { commitAndPush, pullOrClone, writeFile, type RepoConfig } from '../git/ops.js';
import type { MCPServer, Skill } from '../protocol.js';

const HOME = os.homedir();
const SKILL_DIRS = [path.join(HOME, '.anyprov', 'skills'), path.join(HOME, '.claude', 'skills')];
    ...
    for (const id of entries) {
      ...
      try {
        ...
        out.push({
          ...
          isEnabled: true,
          frontmatter: { name: name ?? id, description: description ?? '' } as Record<
            string,
            string
          >,
        });
