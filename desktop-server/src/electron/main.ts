...
function showPairingCodeWindow(code: string): void {
  ...
  if (codeWindow && !codeWindow.isDestroyed()) {
    codeWindow.focus();
    codeWindow.webContents
      .executeJavaScript(`document.getElementById('code').textContent=${JSON.stringify(spaced)}`)
      .catch(() => undefined);
    return;
  }
...
function createWindow(): void {
  ...
  wc.on('did-fail-load', (_e, errorCode, errorDescription, validatedURL, isMainFrame) => {
    ...
    console.error(
      `[apc] renderer failed to load: ${validatedURL} — ${errorCode} ${errorDescription}`
    );
  });
  wc.on('render-process-gone', (_e, details) => {
    console.error(
      `[apc] renderer process gone: reason=${details.reason} exitCode=${details.exitCode}`
    );
  });
...
function registerIpc(): void {
  ...
  ipcMain.handle(
    ...
    async (_e, opts: { enabled: boolean; provider?: Prefs['remoteAccessProvider'] }) => {
  ...
  ipcMain.handle('marketplace:setSourceEnabled', async (_e, id: string, enabled: boolean) => {
  ...
  ipcMain.handle('metal:download', async (event, hubID: string) => {
    ...
    try {
      return await getMetalStore().download(
        hubID,
        (p) => {
  ...
  ipcMain.handle(
    ...
    async (_e, req: { system?: string; user: string; maxTokens?: number }) => {
...
async function promptKillConflictingInstances(port: number): Promise<boolean> {
  ...
  console.log(`[apc] killing conflicting processes: ${conflicts.map((c) => c.pid).join(', ')}`);
