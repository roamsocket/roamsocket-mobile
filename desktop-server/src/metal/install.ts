...
import { metalRuntimeRoot, managedMetalPythonPath, managedMetalPythonMarker } from './paths.js';
 ...
 */
export async function installMetalRuntime(
  onLog: InstallLog = () => undefined
): Promise<MetalInstallResult> {
  ...
  const pip = await runCmd(venvPython, ['-m', 'pip', 'install', '--upgrade', 'mlx-lm'], onLog, {
    timeoutMs: 900_000,
  });
  if (pip.code !== 0) {
    const msg =
