...
export async function startOAuthFlow(
  ...
  return new Promise((resolve) => {
    ...
    const server = http.createServer((req, res) => {
      ...
      if (oauthError) {
        res.end(
          `<html><body>Connection failed: ${escapeHtml(oauthError)}. You can close this tab.</body></html>`
        );
      ...
      res.end(
        `<html><body>Connected to ${escapeHtml(def.name)}! You can close this tab and return to RoamSocket.</body></html>`
      );
...
export async function refreshAccessTokenIfNeeded(
  ...
  const stillValid =
    stored.accessToken && (!stored.expiresAt || stored.expiresAt > Date.now() + 30_000);
