...
function normalizeRecord(raw: unknown): CustomProvider | null {
  ...
  const defaultModel =
    typeof o.defaultModel === 'string' && o.defaultModel.trim() ? o.defaultModel.trim() : undefined;
...
export function findCustomProvider(
  ...
  const slug =
    customSlugFromProviderId(providerIdOrSlug) ?? providerIdOrSlug.replace(/^custom:/, '').trim();
...
export function chatRequestTarget(
  ...
  | { style: 'openai' | 'anthropic' | 'google' | 'metal'; url: string }
  | { style: 'error'; message: string } {
