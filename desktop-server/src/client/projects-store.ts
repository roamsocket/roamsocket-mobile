  ...
  load(): void {
    try {
      ...
      this.projects = Array.isArray(parsed.projects)
        ? parsed.projects
            .filter(
              (p): p is Partial<ProjectItem> & { id: string; name: string } =>
...
export function isMemoryEmptyStateChrome(text: string): boolean {
  ...
  const withoutChrome = n
    ...
    .replace(
      new RegExp(MEMORY_EMPTY_STATE_MARKER.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'gi'),
      ' '
    )
    .replace(
...
export function memoryPlaceholderAt(index: number): string {
  const i =
    ((index % MEMORY_EDIT_PLACEHOLDERS.length) + MEMORY_EDIT_PLACEHOLDERS.length) %
