...
export function buildTranscriptSnippet(messages: NormalizedMessage[], maxChars = 14_000): string {
  const lines: string[] = [];
...
function parseEvalReply(text: string): { met: boolean; reason: string } {
  ...
  const reason =
    lines
      ...
      .join(' ')
      .replace(/^(reason:\s*)/i, '')
      .trim() || (met ? 'Condition appears satisfied.' : 'Condition not yet satisfied.');
