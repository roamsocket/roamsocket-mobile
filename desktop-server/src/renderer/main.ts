...
function showRoute(name: AppView, parts?: string[]) {
  ...
  for (const a of Array.from(document.querySelectorAll('.nav-item'))) {
    ...
    a.classList.toggle('active', route === name || (name === 'metal' && route === 'settings'));
  }
...
function hasUsableChatModel(): boolean {
  const hasKey =
    state.provider === 'localMetal' ? true : !!state.secrets?.providerKeys[state.provider]?.present;
...
async function refreshMetalDownloadedCache(): Promise<void> {
  try {
    ...
    state.metalDownloadedIds = new Set(entries.filter((e) => e.downloaded).map((e) => e.hubID));
    if (
...
function renderChat() {
  ...
  view.innerHTML = '';
  const project = state.projectFilter != null ? projects.get(state.projectFilter) : undefined;
  ...
  const title =
    project?.name ??
    (thread && thread.messages.some((m) => m.role === 'user') ? thread.title : 'New chat');
  if (project) {
    setTopbar({
      title: project.name,
  ...
  if (isEmpty && project) {
    ...
    for (const m of thread!.messages) {
      const isLiveEmptyAssistant =
        state.chatBusy && m.role === 'assistant' && m.id === lastMsg?.id && !m.content.trim();
    ...
    queueMicrotask(() => {
      ...
      if (targetId) {
        const target = msgs.querySelector(
  ...
  if (project) {
    ...
    instr.append(
      el('p', {}, [
        project.description ||
          'Add project notes in Settings later — chats in this project share this scope.',
      ])
    );
    rail.append(instr);
    ...
    ctx.append(el('p', {}, [`${chatCount} chat${chatCount === 1 ? '' : 's'} in this project`]));
...
function buildChatComposer(): HTMLElement {
  ...
  const labelText = usable ? `${modelPillLabel()} · ${effortLabel}` : '+ Add a model';
...
async function populateModelPickerList(listHost: HTMLElement, onPicked: () => void): Promise<void> {
  listHost.innerHTML = '';
  let anyModel = false;
  ...
  for (const p of catalog) {
    ...
    const toggle = makeSubmenuToggle(custom ? `${p.label} (custom)` : p.label, '', body, {
      open: state.provider === p.id,
      level: 'provider',
    });
    ...
    try {
      ...
      loading.remove();
      const countEl = toggle.querySelector('.model-picker-toggle-count') as HTMLElement | null;
      if (models.length === 0) {
      ...
      if (p.id === 'openrouter') {
        ...
        const selectedOrg = selectedIsOpenRouter ? state.model.split('/')[0] || '' : '';
        for (const [org, orgModels] of sorted) {
          const orgBody = el('div', { class: 'model-picker-submenu nested' });
          const orgToggle = makeSubmenuToggle(prettifyName(org), `${orgModels.length}`, orgBody, {
  ...
  if (missingKeys.length > 0 || customsList().length === 0) {
    ...
    add.append(
      el('div', { class: 'sub' }, ['Add an API key or custom provider in Settings → Providers'])
    );
  ...
  try {
    ...
    if (ready.length === 0) {
      ...
      for (const e of ready) {
        ...
        left.append(el('div', { class: 'sub' }, [`${e.family} · ${e.approxSize} · downloaded`]));
        opt.append(left);
    ...
    listHost.append(el('div', { class: 'settings-hint' }, ['Metal catalog unavailable']));
  }

  if (
    !anyModel &&
    missingKeys.length === CHAT_PROVIDERS.filter((p) => p.id !== 'localMetal').length
  ) {
...
async function onChatSend(ta: HTMLTextAreaElement) {
  ...
  if (state.provider !== 'localMetal') {
    ...
    if (state.provider.startsWith('custom:') && !customOk) {
      state.chatError =
        'This custom provider is missing a base URL. Fix it in Settings → Providers.';
      renderChat();
      return;
    }
  } else if (!state.model) {
...
function fullOrError(thread: ReturnType<HistoryStore['get']>, err: string): string {
...
function renderProjectDetail(projectId: string) {
  ...
  if (!project) {
    setTopbar({ title: 'Projects' });
    view.append(el('div', { class: 'page' }, [el('p', { class: 'lead' }, ['Project not found.'])]));
    return;
  }
  ...
  if (project.description) {
    ...
    hero.append(
      el('p', { class: 'hint' }, ['Add a description so chats in this project share context.'])
    );
  }
  mainCol.append(hero);
  ...
  if (projectChats.length === 0) {
    recentWrap.append(el('div', { class: 'settings-hint' }, ['No chats in this project yet.']));
  } else {
  ...
  const editInstr = el(
    'button',
    { class: 'rail-icon-btn', type: 'button', title: 'Set instructions' },
    ['+']
  );
  ...
  fileInput.addEventListener('change', async () => {
    ...
    for (const file of files) {
      ...
      try {
        if (
          file.type.startsWith('text/') ||
          /\.(txt|md|markdown|json|csv|ts|js|swift)$/i.test(file.name)
        ) {
          content = await file.text();
        } else {
  ...
  if (project.contextItems.length === 0) {
    ...
    drop.append(el('p', {}, ['Upload, paste text, or pick a resource from a connector.']));
    ...
    const more = el('button', { class: 'context-add-more', type: 'button' }, [
      '+ Add more context',
    ]);
...
function renderArtifacts() {
  ...
  if (items.length === 0) {
    ...
    for (const a of items) {
      ...
      card.append(
        el('div', { class: 'tldr-body' }, [preview + (a.content.length > 220 ? '…' : '')])
      );
      card.append(el('div', { class: 'art-title' }, [a.title]));
      card.append(
        el('div', { class: 'art-meta' }, [`Edited ${new Date(a.createdAt).toLocaleDateString()}`])
      );
...
async function ensurePairedAndConnected(): Promise<{ token: string; url: string } | null> {
  ...
  const host =
    state.bootstrap.serverHost === '0.0.0.0' || !state.bootstrap.serverHost
      ? '127.0.0.1'
...
function paintGoalBanner(msg: {
  ...
  host.innerHTML = '';
  const title = status === 'achieved' ? 'Goal achieved' : '◎ /goal active';
...
function filterCodeSlashCommands(raw: string): Array<{ token: string; detail: string }> {
  ...
  return CODE_SLASH_COMMANDS.filter(
    (c) =>
      c.token.toLowerCase().startsWith(q) || (q.startsWith('/g') && c.token.startsWith('/goal'))
  );
}
...
function paintPrBar(chip: PrChipModel): void {
  ...
  if (chip.branch) {
    ...
    left.append(
      el('span', { class: 'pr-chip-branch muted' }, [
        chip.url.replace(/^https:\/\/github\.com\//, ''),
      ])
    );
  }
  prBar.append(left);
  ...
  try {
    ...
    if (state.codeSessionLocalId) {
      codeSessions.update(state.codeSessionLocalId, {
        ...
        status: live.state === 'merged' || live.state === 'closed' ? 'done' : 'ready_for_review',
      });
    }
    const rec = state.codeSessionLocalId ? codeSessions.get(state.codeSessionLocalId) : undefined;
...
function renderCodeHome(view: HTMLElement) {
  ...
  if (prs.length === 0) {
    prSec.append(
      el('p', { class: 'settings-hint' }, ['PRs opened by coding sessions will show up here.'])
    );
  } else {
    const list = el('div', { class: 'code-session-list' });
  ...
  modelBtn.addEventListener('click', () => {
    void (async () => {
      ...
      if (state.code.provider && !providerOptions.some((o) => o.value === state.code.provider)) {
...
function renderCodeSession(view: HTMLElement, preserved: Node[]) {
  ...
  const sendBtn = el('button', { class: 'primary-btn', id: 'code-send', type: 'button' }, ['Send']);
    ...
    if (state.sessionId) {
      ...
      await sendClient({
        ...
        model: {
          ...
          apiKey: apiKey || 'none',
          ...(codeEp ? { baseUrl: codeEp.baseUrl, apiStyle: codeEp.apiStyle } : {}),
        },
...
function fillSettingsGeneral(panel: HTMLElement): void {
  ...
  addToggle(
    ...
    'allowLanDiscovery',
    'Phones can find this machine on the same Wi‑Fi.'
  );
  addToggle(
    perms,
    'Show pairing code popup',
    'showPairingCodePopup',
    'Large code window when the app starts.'
  );
  ...
  addToggle(
    ...
    'alwaysQuitOnClose',
    'Otherwise closing keeps the server in the tray.'
  );
  panel.append(perms);
}
...
function fillSettingsProviders(panel: HTMLElement): void {
  ...
  for (const prov of CHAT_PROVIDERS) {
    ...
    row.append(
      el('div', { class: `pill ${present ? 'ok' : 'empty'}` }, [present ? 'configured' : 'empty'])
    );
    const setBtn = el('button', { class: 'ghost-btn', type: 'button' }, [
      present ? 'Replace' : 'Add',
    ]);
  ...
  if (customs.length === 0) {
    customSec.append(el('p', { class: 'settings-hint' }, ['No custom providers yet.']));
  }
  for (const prov of customs) {
    ...
    row.append(title);
    row.append(
      el('div', { class: `pill ${present ? 'ok' : 'empty'}` }, [present ? 'key set' : 'no key'])
    );
  ...
  void window.apc.metal
    ...
    .then((st) => {
      installCard.insertBefore(
        el(
          'div',
          { class: `notice ${st.runtimeReady ? 'ok' : 'warn'}`, style: 'margin-bottom:10px' },
          [st.detail]
        ),
        metalActions
      );
...
async function fillSettingsLightweight(panel: HTMLElement): Promise<void> {
  ...
  const saveBtn = el('button', { class: 'primary-btn', type: 'button' }, [
    'Save Lightweight Tasks',
  ]);
...
function fillSettingsConnection(panel: HTMLElement): void {
  ...
  addToggle(
    perms,
    'Auto tunnel after pair',
    'autoTunnelOnPair',
    'Start a public tunnel after phone pair.'
  );
  addToggle(
    perms,
    'Rotate code after pair',
    'rotateCodeAfterPair',
    'Issue a new code after each pair.'
  );
  panel.append(perms);
...
function memoryEntryRow(entry: MemoryEntry): HTMLElement {
  ...
  row.append(el('span', { class: 'memory-entry-time' }, [relativeMemoryTime(entry.updatedAt)]));
...
function openMemoryEntryDetail(entryId: string): void {
  ...
  const paint = () => {
    ...
    modal.append(el('p', { class: 'memory-detail-summary' }, [cur.summary || 'No summary yet.']));
...
function openMemoryImportModal(): void {
  ...
  modal.append(
    el('div', { class: 'memory-import-step' }, [
      ...
      el('div', { class: 'memory-import-step-body' }, [
        el('div', { class: 'memory-import-step-title' }, ['Paste results below to add to memory']),
      ]),
    ])
  );
...
function fillSettingsSkills(panel: HTMLElement): void {
  ...
  if (list.length === 0) {
    sec.append(
      el('div', { class: 'settings-table-row muted' }, [el('span', {}, ['No skills installed'])])
    );
  } else {
  ...
  if (mpSkills.length) {
    sec.append(
      el('div', { class: 'settings-subhead', style: 'margin-top:18px' }, ['From marketplace'])
    );
...
async function fillSettingsMarketplace(panel: HTMLElement): Promise<void> {
  ...
  for (const src of status.sources) {
    ...
    const left = el('div', { style: 'flex:1;min-width:0' });
    left.append(
      el('div', { class: 'settings-skill-name' }, [src.name + (src.isDefault ? ' (default)' : '')])
    );
    left.append(el('div', { class: 'settings-hint', style: 'word-break:break-all' }, [src.url]));
    if (src.lastError) {
      left.append(
        el('div', { class: 'settings-hint', style: 'color:var(--warn,#e8a838)' }, [src.lastError])
      );
  ...
  add.addEventListener('click', async () => {
    const values = await appForm({
      ...
      message: 'Raw catalog.json URL, github.com blob/tree link, or owner/repo.',
      fields: [
        {
          name: 'url',
...
function fillSettingsConnectors(panel: HTMLElement): void {
  ...
  if (catalog.length === 0) {
    ...
    for (const c of catalog) {
      ...
      const badge = el('span', { class: `pill ${c.available === false ? 'empty' : 'ok'}` }, [
...
function startMetalDownload(hubID: string, displayName: string): void {
  ...
  void window.apc.metal
    ...
    .catch((err) => {
      ...
      if (cur) {
        cur.phase =
          controller.signal.aborted || message.includes('cancelled') ? 'cancelled' : 'error';
...
function paintMetalProgressBanner(): void {
  ...
  const tracks = activeMetalDownloads().filter(
    (t) =>
      ...
      t.phase === 'error' ||
      t.phase === 'cancelled' ||
      (t.phase === 'done' && Date.now() - t.updatedAt < 5000)
  );
  if (tracks.length === 0) {
    host.classList.add('hidden');
    return;
  ...
  for (const t of tracks) {
    ...
    bar.append(fill);
    row.append(bar);
    row.append(el('div', { class: 'metal-progress-status' }, [t.error ? t.error : t.status]));
    if (t.phase === 'active') {
...
function renderMetalBrowseRoot(
  ...
  if (featured.length === 0) {
    featuredSec.append(
      el('p', { class: 'settings-hint' }, ['No featured families in the catalog.'])
    );
  } else {
  ...
  buckets.append(
    metalBucketLink('legacy', 'Legacy models', 'Older or larger variants that may be limited.')
  );
  buckets.append(
    metalBucketLink(
      'experimental',
...
function metalVariantRow(e: MetalEntry, showFamily: boolean): HTMLElement {
  ...
  if (e.downloaded) {
    ...
    const dl = el('button', { class: 'primary-btn sm', type: 'button' }, ['Download']);
...
async function refreshRemoteAccess(container: HTMLElement): Promise<void> {
  ...
  row.append(title);
  row.append(
    el('div', { class: `pill ${ra.enabled ? 'ok' : 'empty'}` }, [ra.enabled ? 'on' : 'off'])
  );
  ...
  if (ra.url) {
    container.append(
      el('div', { class: 'settings-hint', style: 'margin-top:10px' }, [
...
async function main() {
  ...
  if (prefs) {
    for (const [provider, pref] of Object.entries(prefs)) {
      ...
      const hasKey = provider === 'localMetal' || !!state.secrets?.providerKeys[provider]?.present;
      if (
