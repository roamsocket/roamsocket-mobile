...
function clearSkillPopovers(): void {
  document.querySelectorAll(".skill-hover-preview, .skill-overflow-menu").forEach((n) => n.remove());
}
...
export function buildSkillSlashChip(
  ...
  const showPreview = () => {
    ...
    const clipped =
      body.length > 220 ? body.replace(/\s+/g, " ").trim().slice(0, 217) + "…" : body;
...
export function openSkillDetailModal(
  ...
  infoBtn.addEventListener("click", (e) => {
    ...
    tip.append(
      el("div", { class: "skill-info-k", style: "margin-top:8px" }, ["Description"]),
    );
