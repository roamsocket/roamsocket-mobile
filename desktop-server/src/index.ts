...
export async function startServer(opts: StartServerOptions = {}): Promise<RunningServer> {
  ...
  // E2B sandbox runs are now phone-only: the iOS / Android apps talk
  // to e2b.dev directly with the user's own API key. The desktop no
  // longer auto-triggers sandboxes after `git_publish` and no longer
  // proxies run/start/abort messages — the corresponding WS handlers
  // were removed from this file. See `ios/AnyProvCore/.../Sandboxes`
  // for the phone-side runner.
  const app = express();
  ...
  wss.on('connection', (ws: WebSocket, req) => {
    ...
    // E2B sandbox runs are now phone-only — the desktop no longer
    // owns the runner or auto-triggers one after `git_publish`. The
    // phone drives the sandbox directly via e2b.dev. There is no
    // per-connection E2B state to track here anymore.
      ...
      try {
        switch (msg.type) {
          ...
          case 'create_pr':
            ...
            break;
          case 'git_publish': {
            // E2B sandbox runs used to auto-trigger here against the
            // pushed workdir. The runner is now phone-only — the
            // iOS / Android app drives sandboxes directly against
            // e2b.dev with the user's own key, so there's nothing for
            // the desktop to do after a successful push. The pushed
            // branch is still in `result.repoFullName` /
            // `result.branch` and the phone can pick it up via the
            // Sandboxes "Start a run" sheet.
