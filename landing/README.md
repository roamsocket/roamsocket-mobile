...
Static marketing site for **RoamSocket**. No build step — HTML, CSS, and JS under `public/` are served as [Workers static assets](https://developers.cloudflare.com/workers/static-assets/).
...
In the [Cloudflare dashboard](https://dash.cloudflare.com/) → **Workers & Pages** → **roamsocket** → **Settings** → **Domains & Routes**, attach a custom domain. DNS must be on the same Cloudflare account.
