...
final class DirectE2BClientTests: XCTestCase {
    ...
    /// MiniMax M3 has shifted to a key-value body shape:
    /// `[tool_call: name]\nid=call_…\ninput={"command":"…"}\n</tool_call>`.
    /// The `input=` value is a JSON object that already matches
    /// the tool's parameter schema, so it must be splatted into
    /// the top level of the input map (not nested under
    /// `input`). Without this the runner never sees the
    /// `command` field and the tool call is dropped on the
    /// floor as a "missing 'command' field" error.
    func testParseProviderTextToolCallsKeyValueShape() {
        let raw = """
        <tool_call>[tool_call: run_shell]
        ...
        XCTAssertEqual(
            ...
            "command must be extracted from the JSON-shaped input value; argumentsJSON=\(parsed.toolCalls[0].argumentsJSON)"
        )
